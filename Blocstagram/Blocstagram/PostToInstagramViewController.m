...
@property (nonatomic) NSOperationQueue *photoFilterOperationQueue;
@property (nonatomic) UICollectionView *filterCollectionView;

@property (nonatomic) NSMutableArray *filterImages;
@property (nonatomic) NSMutableArray *filterTitles;

@property (nonatomic) UIButton *sendButton;
@property (nonatomic) UIBarButtonItem *sendBarButton;
...
#pragma mark - Attributed String For Button Title
...
- (void)addFiltersToQueue {
    ...
    // Aqua filter
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *aquaFilter = [CIFilter filterWithName:@"CIPhotoEffectProcess"];
        
        if (aquaFilter) {
            [aquaFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            [self addCIImageToCollectionView:aquaFilter.outputImage withFilterTitle:NSLocalizedString(@"Aqua", @"Aqua Filter")];
        }
    }];
    ...
    // the Drunk filter: tilt the image 0.2 radians (about 11.5 degrees), then make a blurry double-vision effect
    [self.photoFilterOperationQueue addOperationWithBlock:^{
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex != alertView.cancelButtonIndex) {
        ...
        if (!success) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Couldn't save image", nil)
